...
GSepSteer.GeneratorKit = function() {}
GSepSteer.GeneratorKit.prototype.identity = function() {
	return ('Gen02.SeparateSteer.GeneratorKit ()');
};
GSepSteer.GeneratorKit.prototype.init = function() {
	this.generator = null;
	
	this.moveTrans = [ {x:0,y:-1},{x:1,y:0},{x:0,y:1},{x:-1,y:0} ];	
};
GSepSteer.GeneratorKit.prototype.close = function() {
	this.generator = null;

	this.moveTrans = null;
};

GSepSteer.GeneratorKit.prototype.getClearDirection = function(height, point) {
	for(var i=0; i<4; i++)
	{
		var newpt = {x:0,y:0};
...
GSepSteer.GeneratorKit.prototype.setDescending = function(randomPoint, height) {
...
GSepSteer.GeneratorKit.prototype.setHotspot = function(randomPoint, dir, room, height) {
	var logic = false;
...
GSepSteer.GeneratorKit.prototype.isInRange = function(point, height) {
	var logical = true;
...
GSepSteer.GeneratorKit.prototype.overlapsVisited = function(box, height) {
...
GSepSteer.GeneratorKit.prototype.getOpenDirections = function(point, height) {
	var dirList = [];
	var targetV = {x:0,y:0};
...
GSepSteer.GeneratorKit.prototype.getOpenAreas = function(point, size, dir, h) {
	var boxList = [];
	var topLeft = {x:0,y:0};
	var steps = {x:0,y:0};
	var totalSteps = 0;
...
GSepSteer.GeneratorKit.prototype.getAllAreas = function(point, dir, h) {
	var boxList = [];
...
GSepSteer.GeneratorKit.prototype.getOpenDistance = function(point, dir, h) {
	var count=0;
...
GSepSteer.GeneratorKit.alloc = function() {
	var vc = new GSepSteer.GeneratorKit();
	vc.init();
	return vc;
};
